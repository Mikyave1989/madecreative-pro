import { l as logger, D as DEFAULT_MODEL, P as PROVIDER_LIST } from './server-build-BTKZv3hM.js';
...
const getLogStore = () => {
  if (!logStore && typeof window !== "undefined") {
    try {
      import('./server-build-BTKZv3hM.js').then(n => n.c).then(({ logStore: store }) => {
        logStore = store;
      }).catch(() => {
      });
    } catch {
...
class DebugLogger {
  ...
  async _getLogStoreLogs() {
    try {
      ...
      if (!store) {
        try {
          const { logStore: storeModule } = await import('./server-build-BTKZv3hM.js').then(n => n.c);
