...
// Only expose Anthropic as the LLM provider for MadeCreative
const _allProviders = llmManager.getAllProviders();
export const PROVIDER_LIST = _allProviders.filter((p) => p.name === 'Anthropic');
export const DEFAULT_PROVIDER = PROVIDER_LIST[0] || llmManager.getDefaultProvider();
